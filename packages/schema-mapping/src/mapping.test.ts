...
const superset = {
  ns: {
    ...
    mappings: { actions: { appsync: { path: 'info.fieldName' }, apiGateway: { path: 'requestContext.httpMethod' } } },
  },
}
...
describe('extractFromApiGateway (per-action)', () => {
  it('extracts vars from URL template and builds resource', () => {
    ...
    const { action, resource, vars } = extractFromApiGateway(ev, superset as any)
    expect(action).toBe('getTenantGrant')
    expect(vars).toMatchObject({ tenantId: 'acme', userId: 'alice' })
    expect(resource).toEqual({ entityType: 'TenantGrant', entityId: 'acme:alice' })
  })
})
