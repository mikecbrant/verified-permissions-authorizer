...
const pruneForCedar = (doc: Json): void => {
  ...
  if (b.entityTypes && typeof b.entityTypes === 'object') {
    for (const [, et] of Object.entries<any>(b.entityTypes)) {
      if (et && typeof et === 'object' && et.resourceEntities) delete et.resourceEntities
    }
  }
...
const mergeCedarSchemas = (baseYaml: string, partialYaml: string): MergeResult => {
  ...
  if (pbody?.entityTypes) {
    ...
    for (const [name, pdef] of Object.entries<any>(pbody.entityTypes)) {
      ...
      if (typeof pdef !== 'object' || pdef == null) throw new Error(`entityTypes.${name} must be an object`)
      for (const [k, v] of Object.entries<any>(pdef)) {
        ...
        for (const [tpl, tdef] of Object.entries<any>(v ?? {})) {
          if (bdef.resourceEntities[tpl]) throw new Error(`cannot override existing resourceEntities template ${name}.${tpl}`)
  ...
  if (pbody?.actions) {
    ...
    for (const [name, pdef] of Object.entries<any>(pbody.actions)) {
      ...
      if (typeof pdef !== 'object' || pdef == null) throw new Error(`actions.${name} must be an object`)
      for (const [k, v] of Object.entries<any>(pdef)) {
        ...
        if (k === 'entityMap') {
          ...
          for (const [resType, tpl] of Object.entries<any>(v ?? {})) {
            if (bdef.entityMap[resType]) throw new Error(`cannot override existing actions.${name}.entityMap for ${resType}`)
...
const validateSuperset = (doc: Json): string[] => {
  ...
  for (const [aname, adef] of Object.entries<any>(actions)) {
    ...
    for (const rt of rtypes) {
      ...
      if (!tplName) {
        errors.push(`actions.${aname}.entityMap missing key for resourceType ${rt}`)
        continue
      }
      const tpl = entityTypes?.[rt]?.resourceEntities?.[tplName]
      if (!tpl) {
        errors.push(`actions.${aname}.entityMap.${rt} references missing template ${rt}.resourceEntities.${tplName}`)
      } else {
        // Variable coverage per integration
        const needed = new Set([
          ...listTemplateVars(tpl.id),
          ...Object.values(tpl.attributes ?? {}).flatMap((v: any) => listTemplateVars(v)),
        ])
        const appsyncVars = new Set(Object.keys(adef?.input?.appsync?.body ?? {}))
        const restUrlVars = new Set(urlTemplateVars(adef?.input?.rest?.url))
        const restBodyVars = new Set(Object.keys(adef?.input?.rest?.body ?? {}))
        ...
        for (const v of needed) {
          if (!(restUrlVars.has(v) || restBodyVars.has(v) || restQueryVars.has(v))) {
            errors.push(
